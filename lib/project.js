...
module.exports = {
    ...
    useCustomConfig: (config) => {
        ...
        if (config.bundles.length) {
            module.exports.installBundles(config, sf);
        }
    },
    ...
    installBundles: (config, sf) => {
        let items = [], devItems = [];

        config.bundles.forEach(function(bundle) {
            bundle[1] ? devItems.push(bundle[0]) : items.push(bundle[0]);
        });

        module.exports.projectCommand(config, `composer config extra.symfony.allow-contrib true`);
        module.exports.installLibs(config, sf, items.join(' '));
        module.exports.installDevLibs(config, sf, devItems.join(' '));
        module.exports.projectCommand(config, `composer config extra.symfony.allow-contrib false`);
    },
};
